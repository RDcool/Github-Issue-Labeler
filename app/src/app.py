...
app.layout = html.Div([
    ...
    dbc.Popover(
        "Введёный текст не является ссылкой на issue в GitHub",
        id="input_popover",
        body=True,
        target="link_input",
        placement="bottom",
...
# Функция отображения popover-а
@app.callback(
    Output("input_popover", "is_open"),
    Output("input_popover", "children"),
    Input("link_input", "invalid"),
    State("link_input", "value")
)
def show_popover(input_flag, input_value):
    pattern1 = r'(https:\/\/([A-z1-9]+)\.([a-z]+))'
    pattern2 = r'(https:\/\/(www)\.([A-z1-9]+)\.([a-z]+))'
    if input_flag:
        if bool(re.match(pattern1, input_value)) or bool(re.match(pattern2, input_value)):
            return True, "Введённая ссылка не является ссылкой на issue в GitHub"
        else:
            return True, "Введённый текст не является ссылкой"
    else:
        return False, ""
